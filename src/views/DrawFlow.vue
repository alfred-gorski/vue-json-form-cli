...
import { onMounted, shallowRef, h, getCurrentInstance, render, ref, inject } from 'vue'
...
function exportGraph() {
	...
	for (let key in data) {
		...
		for (let name of names) {
			content += '  ' + nodeName.split(' ').join('_') + '->' + name.split(' ').join('_') + '\n';
		}
	}
	content += '}\n'
...
function addNodeToDrawFlow(id, pos_x, pos_y) {
	...
	editor.value.addNode(node, 1, 1, pos_x, pos_y, node, nodeSelected, node, 'vue');
